...
export default function SignupPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<"internship" | "fulltime" | "">("");
          ...
          <div className="hidden lg:block">
            ...
            <div className="space-y-6">
              ...
              <p className={`text-lg ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`}>
                Join thousands of students who are organizing their job search 
                and landing their dream opportunities with OneRoute.
              </p>
              ...
              <div className="pt-8">
                ...
                <div className="flex flex-wrap gap-4">
                  {["MIT", "Stanford", "Harvard", "UC Berkeley"].map((uni) => (
                    <div
                      key={uni}
                    ...
                    >
                      <span className="font-semibold text-sm">{uni}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            >
              ...
              <div className="mb-8">
                ...
                <p className={`${theme === "dark" ? "text-slate-400" : "text-slate-600"}`}>
                  Get started with OneRoute for free
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    {[
                      ...
                      return (
                        <button
                          key={option.value}
                          type="button"
                          onClick={() => setSelectedRole(option.value as "internship" | "fulltime")}
                          className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-medium transition-all duration-300 border ${
                            selectedRole === option.value
                              ? "bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white border-transparent shadow-lg scale-105"
                              : theme === "dark"
                              ? "bg-slate-900/50 border-slate-700 hover:bg-slate-900 text-white hover:scale-105"
                              : "bg-white border-slate-200 hover:bg-slate-50 text-slate-900 hover:scale-105"
                          }`}
                        >
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <span className={`text-sm ${theme === "dark" ? "text-slate-300" : "text-slate-700"}`}>
                    I agree to the{" "}
                    <Link href="/terms" className="text-violet-600 hover:text-fuchsia-600 font-medium transition-colors">
                      Terms of Service
                    </Link>{" "}
                    and{" "}
                    <Link href="/privacy" className="text-violet-600 hover:text-fuchsia-600 font-medium transition-colors">
                      Privacy Policy
                    </Link>
                  </span>
                </label>
